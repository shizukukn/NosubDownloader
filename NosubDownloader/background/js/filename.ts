...
/// <reference path="../../../typings/http-string-parser/http-string-parser.d.ts" />
/// <reference path="../../typings/content-type.d.ts" />
...
function isVideoResponse(details: chrome.webRequest.OnHeadersReceivedDetails): boolean {
    var statusLine = parser.parseStatusLine(details.statusLine);
...
chrome.runtime.onMessage.addListener(
    (message: nosub.Message, sender: chrome.runtime.MessageSender, sendResponse: Function) => {
        var tabId = sender.tab.id;

        switch (message.type) {
            // 動画ページが開かれた場合、そのタブを監視対象にする
            case nosub.OPENED_VIDEO_PAGE_MESSAGE_TYPE:
            ...
            // (メモリリーク阻止 & フィルタ処理高速化)
            case nosub.CLOSED_VIDEO_PAGE_MESSAGE_TYPE:
                trace(message, 'tadId = ' + tabId);
                videoPageTabs[tabId] = undefined; // delete
                break;

            // 動画リンクが追加された時の処理
            case nosub.ADD_VIDEO_URLS_MESSAGE_TYPE:
                var addVideoUrlsMessage = <nosub.AddVideoUrlsMessage>message;
